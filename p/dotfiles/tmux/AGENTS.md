...
When choosing colors for styling (status line, pane borders, or any other tmux element), always use named ANSI colors (`black`, `red`, `green`, `yellow`, `blue`, `magenta`, `cyan`, `white`, and their `bright*` variants) rather than fixed 256-color palette values (e.g. `colour238`). Named ANSI colors resolve against Jeff's terminal's configured theme, while `colourNNN` values are fixed RGB entries that ignore it. Prefer this even when a `colourNNN` value seems like a closer visual match.
...
When a tmux configuration session appears complete, ask whether Jeff wants to back up the change to the dotfiles bare repo (`~/.tmux.conf` is tracked there). Do not run the backup commit or push until Jeff confirms. This directory sits under `~/p/dotfiles/`, so the `/dotfiles-sync` command and general sync guidance in `~/p/dotfiles/AGENTS.md` are already inherited here — for the underlying bare-repo mechanics, safety rules, and branch model, see `~/.dotfiles/AGENTS.md`, the canonical home for that.
